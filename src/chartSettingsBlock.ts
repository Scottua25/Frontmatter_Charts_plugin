...
export async function renderChartSettingsBlock(
	...
	const colorscaleSetting = new Setting(block)
		...
		.addDropdown((drop) => {
			...
			drop.setValue(config.colorscale || "YlGnBu").onChange(async (val) => {
	...
	const reverseScaleSetting = new Setting(block)
		...
		.addToggle((toggle) => {
			toggle.setValue(config.reverseScale ?? false).onChange(async (val) => {
	...
	addInlineColorPicker(
		...
		async (val) => {
			config.chartColor = val;
			const textField = chartColorSetting.controlEl.querySelector(
				"input[type='text']"
			) as HTMLInputElement;
